...
export async function HasGlobalCommands(appId, guildId, commands) {
    if (guildId === '' || appId === '') return;

    commands.forEach((c) => HasGlobalCommand(appId, guildId, c));
}

// Checks for a command
async function HasGlobalCommand(appId, guildId, command) {
    // API endpoint to get and post guild commands
